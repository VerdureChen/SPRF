...
def define_dsearch_args(parser):
    ...
    parser.add_argument('--encoder-class', type=str,
    ...
    parser.add_argument('--total_prf_docs', type=int, metavar='total num of passages used for PRF test', required=False, default=1000,
    ...
    parser.add_argument('--rocchio-gamma', type=float, metavar='gamma parameter for rocchio', required=False,
                        default=0.1,
                        help="The gamma parameter to control the contribution from the average vector of the negative PRF passages")
    parser.add_argument('--rocchio-topk', type=int, metavar='topk passages as positive for rocchio', required=False,
                        default=3,
                        help="Set topk passages as positive PRF passages for rocchio")
    parser.add_argument('--rocchio-bottomk', type=int, metavar='bottomk passages as negative for rocchio',
...
def dsprf_main():
    ...
    query_encoder = init_query_encoder(
        args.encoder, args.encoder_class, args.tokenizer, args.topics, args.encoded_queries, args.device,
        args.query_prefix)
    if args.pca_model:
    ...
    if os.path.exists(args.index):
        ...
        if args.searcher.lower() == 'bpr':
            ...
            searcher = FaissSearcherSplitPRF(args.index, query_encoder)
    else:
        ...
        if args.searcher.lower() == 'bpr':
            ...
            searcher = FaissSearcherSplitPRF.from_prebuilt_index(args.index, query_encoder)

    if not searcher:
        exit()

    # Check PRF Flag
    if args.prf_depth > 0 and type(searcher) == FaissSearcherSplitPRF:
        PRF_FLAG = True
        ...
        print(f'Running FaissSearcherPRF with {args.prf_method.upper()} PRF...')
    else:
        PRF_FLAG = False
