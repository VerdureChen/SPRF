...
def run_cli(config, initial_name, initial_run, prf_method):
    ...
    output_name = f"{Path.cwd().parents[0]}/output/sparse_position_{prf_method}_randprf/{config['topics']}" \
                  f"/run.{config['total_prf_docs']}.{config['split_num']}.{config['prf_depth']}.{config['nsplit']}.{config['seed']}.trec"
    log_name = f"{Path.cwd().parents[0]}/logs/sparse_position_{prf_method}_randprf/{config['topics']}" \
...
def sweep_sparse_position(
        initial_run: str = "/home1/cxy/A-SPRF/Baselines/run.msmarco-v1-passage.ance-otf.dl19.txt",
        mode: Literal["random_split", "natural","total_random"] = "total_random",
